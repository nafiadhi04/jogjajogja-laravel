<section class="gallery-section">
    <div class="gallery-container">
        ...
        <div x-data="{
            ...
            originalSlides: {{ $wisata->gambar->count() + 1 }}, 
            // Total slide di DOM: originalSlides + 2 kloningan
            totalSlides: {{ $wisata->gambar->count() + 1 + 2 }}, 
            ...
            handleLoopTransition() {
                ...
                // Cek apakah BARU SAJA PINDAH ke kloningan terakhir (totalSlides - 1)
                if (this.activeSlide === this.totalSlides - 1) { 
                    targetSlide = 1; // Kembali ke slide ASLI pertama
                } 
                // Cek apakah BARU SAJA PINDAH ke kloningan pertama (indeks 0)
                else if (this.activeSlide === 0) { 
                    targetSlide = this.originalSlides; // Kembali ke slide ASLI terakhir
                } 
                
                if (targetSlide !== -1) {
                    // Beri waktu 400ms agar transisi ke slide kloningan selesai
                    setTimeout(() => {
            ...
            startDrag(e) {
                ...
                this.isTransitioning = false; // Matikan transisi saat drag agar geseran responsif
            },
            ...
            endDrag() {
                ...
                // 💡 PERBAIKAN: Nyalakan transisi HANYA untuk efek bounce/pindah
                this.isTransitioning = true; 
                ...
                // Klamping ke batas DOM (0 atau totalSlides - 1)
                ...
                this.scrollToActiveThumbnail(originalIndex);
                // Panggil handleLoopTransition untuk mengatur timing teleport setelah transisi smooth selesai
                this.handleLoopTransition(); 
            },
            ...
            <div class="relative overflow-hidden rounded-lg shadow-md">
                <div class="cursor-grab"
                    ...
                    @touchend="endDrag()">
                    <div x-ref="slider"
                        class="flex transition-transform duration-400 ease-in-out **will-change-transform**" 
                        ...
                        <div class="flex-shrink-0 w-full main-image-container">
                            <img src="{{ asset('storage/' . ($wisata->gambar->last()->path_gambar ?? $wisata->thumbnail)) }}"
                                alt="Galeri Kloning Terakhir" 
                                class="object-cover w-full h-full"
                                draggable="false"
                                **decoding="async"**> {{-- 👈 PERBAIKAN: Decoding asinkron --}}
                        </div>

                        {{-- 2. Thumbnail sebagai slide ASLI pertama (Index 1) --}}
                        <div class="flex-shrink-0 w-full main-image-container">
                            <img src="{{ asset('storage/' . $wisata->thumbnail) }}"
                                alt="{{ $wisata->nama }}" 
                                class="object-cover w-full h-full"
                                draggable="false"
                                **decoding="async"**> {{-- 👈 PERBAIKAN: Decoding asinkron --}}
