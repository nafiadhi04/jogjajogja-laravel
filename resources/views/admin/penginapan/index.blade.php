                    ...
                    <table class="min-w-full border border-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left border">Views</th> {{-- <-- KOLOM BARU --}} <th
                                    class="px-4 py-3 text-left border">Author</th>
                        ...
                        <tbody>
                            @forelse ($all_penginapan as $item)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-2 border">
                                        <img src="{{ asset('storage/thumbnails_penginapan/' . $item->thumbnail) }}"
                                            alt="{{ $item->nama }}" class="object-cover w-24 h-16 rounded-md">
                                    </td>
                                    <td class="px-4 py-2 border">{{ $item->nama }}</td>
                                    <td class="px-4 py-2 border">{{ $item->kota }}</td>
                                    <td class="px-4 py-2 border">Rp {{ number_format($item->harga) }}</td>
                                    <td class="px-4 py-2 border">{{ $item->views }}</td> {{-- <-- DATA BARU --}} <td
                                        class="px-4 py-2 border">{{ $item->author->name }}</td>
                                        <td class="px-4 py-2 border">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-4 py-12 text-center text-gray-500 border"> {{-- <-- Colspan
                                            diubah menjadi 7 --}} Belum ada data artikel penginapan. </td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
