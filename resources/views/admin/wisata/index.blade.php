...
<x-app-layout>
        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="p-4 bg-white rounded-md shadow-sm">
                ...
                <div class="p-3 mb-4 rounded bg-gray-50">
                    <form action="{{ route('admin.wisata.index') }}" method="GET" class="grid grid-cols-1 gap-3 md:grid-cols-4">
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <a href="{{ route('admin.wisata.index') }}" class="w-full px-3 py-1 text-xs font-semibold text-center text-gray-700 bg-gray-200 rounded hover:bg-gray-300">Reset</a>
                        </div>
                    </form>
                </div>
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-3 py-2 align-top border">
                                        @if($item->status == 'diterima')
                                            <a href="{{ route('wisata.detail', $item->slug) }}" target="_blank" class="text-sm font-medium text-indigo-600 hover:underline">
                                                {{ $item->nama }}
                                            </a>
                                        @else
                                            ...
                                            <div x-show="open" x-cloak x-transition x-ref="menu"
                                                ...
                                                <div class="py-1">
                                                    @if(Auth::user()->role === 'admin' || (Auth::user()->role === 'member' && $item->status === 'revisi'))
                                                        <a href="{{ route('admin.wisata.edit', $item) }}" class="block px-4 py-2 text-sm text-left text-gray-700 hover:bg-gray-100">Edit</a>
                                                    @endif
                                                    ...
                                                    <form action="{{ route('admin.wisata.destroy', $item) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus artikel ini?');">
                                                        @csrf
                                                        @method('DELETE')
            ...
            <div x-show="open" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div x-show="open" x-transition @click.outside="open = false" class="relative w-full max-w-lg bg-white rounded-md shadow-lg">
                    <form method="POST" :action="(() => {
                        const tpl = document.getElementById('route-templates')?.dataset?.wisataAuthorTemplate || '/admin/wisata/__ID__/author';
    ...
    @can('admin')
        <div x-data="{
                ...
                error:null,
                openFor(event){
                    console.log('[wisata-status-modal] openFor', event.detail);
                ...
                buildUrl(){
                    const tpl = document.getElementById('route-templates')?.dataset?.wisataStatusTemplate || '/admin/wisata/__ID__/status';
                    return tpl.replace('__ID__', encodeURIComponent(this.routeKey || ''));
                },
                async submitStatus(){
                    console.log('[wisata-status-modal] submitStatus start', { routeKey: this.routeKey, status: this.status });
                    if (this.loading) {
                        console.log('[wisata-status-modal] already loading');
                        return;
                    }
                    if (!this.routeKey) {
                    ...
                    try {
                        console.log('[wisata-status-modal] fetch PATCH', url, payload);
                        ...
                        console.log('[wisata-status-modal] fetch done', res.status, body);
                        ...
                        console.error('[wisata-status-modal] fetch error', err);
